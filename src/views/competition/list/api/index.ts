...
export const getCompetitionList = (params: CompetitionQueryParams) => {
  return http.request<PageResponse<CompetitionInfo[]>>(
    "get",
    "/api/v1/admin/competition",
    { params }
  );
};
...
// 审核创业大赛
export const reviewCompetition = (
  id: number,
  data: { status: number; reviewComment?: string }
) => {
  return http.request<Response<CompetitionInfo>>(
    "post",
    `/api/v1/admin/competition/${id}/review`,
    { data }
  );
};

// 推荐/取消推荐创业大赛
export const recommendCompetition = (
  id: number,
  data: { isRecommended: boolean }
) => {
  return http.request<Response<CompetitionInfo>>(
    "post",
    `/api/v1/admin/competition/${id}/recommend`,
    { data }
  );
};
