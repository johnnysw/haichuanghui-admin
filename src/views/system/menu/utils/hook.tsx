...
export function useMenu() {
  ...
  const formRef = ref();
  const dataList = ref<Menu.BaseMenu[]>([]);
  ...
  function openDialog(title = "新增", row?: Menu.BaseMenu) {
    addDialog({
      title: `${title}菜单`,
      props: {
        formInline: {
      ...
      beforeSure: async (done, { options }) => {
        ...
        const curData = options.props.formInline as Menu.FormData;
          ...
          if (valid) {
            try {
              if (title === "新增") {
                ...
                message(`新增菜单【${transformI18n(curData.title)}】成功`, {
                  type: "success"
                });
              } else {
                await updateMenu(curData);
                message(`修改菜单【${transformI18n(curData.title)}】成功`, {
                  type: "success"
                });
              }
              done(); // 关闭弹框
  ...
  async function handleDelete(row) {
    try {
      ...
      message(`删除菜单【${transformI18n(row.title)}】成功`, {
        type: "success"
      });
      onSearch();
    } catch (error) {
