    ...
    <div class="px-6 py-6">
      <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="flex-shrink-0">
          <el-image
            ...
            preview-teleported
          />
          <div v-else class="w-32 h-32 lg:w-48 lg:h-32 bg-gray-100 rounded-lg flex items-center justify-center border">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="font-medium">{{ formatDateTime(event.startTime) }}</span>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="font-medium">{{ formatDateTime(event.endTime) }}</span>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <span class="font-medium">
                {{ event.participantCount }}{{ event.maxParticipants ? `/${event.maxParticipants}` : '' }}人
              </span>
            </div>
      ...
      <div v-if="stats && !statsLoading" class="mt-6 pt-6 border-t">
        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4">
          ...
          <div class="text-center">
            <div class="text-2xl font-bold text-green-600">{{ stats.registrations }}</div>
            <div class="text-xs text-gray-500">报名人数</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-purple-600">{{ stats.favorites }}</div>
            <div class="text-xs text-gray-500">收藏数</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-orange-600">{{ stats.shares }}</div>
            <div class="text-xs text-gray-500">分享数</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-red-600">{{ stats.todayViews }}</div>
            <div class="text-xs text-gray-500">今日浏览</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-indigo-600">{{ stats.monthlyViews }}</div>
            <div class="text-xs text-gray-500">月度浏览</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-yellow-600">{{ stats.averageRating.toFixed(1) }}</div>
            <div class="text-xs text-gray-500">平均评分</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-cyan-600">{{ stats.feedbackCount }}</div>
